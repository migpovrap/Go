...
def str_to_intersection(s):
    return new_intersection(s[0], int(s[1:]))

## FAN - ADT intersection
def get_adjacent_intersections(pos, last_pos):
    move = {
        'U': lambda x: 0 if get_row(x) == get_row(last_pos) else new_intersection(get_col(x), get_row(x)+1),
        'D': lambda x: 0 if get_row(x) == 1 else new_intersection(get_col(x), get_row(x)-1),
        'L': lambda x: '' if get_col(x) == 'A' else new_intersection(LETTERS[LETTERS.index(get_col(x))-1], get_row(x)),
        'R': lambda x: '' if get_col(x) == get_col(last_pos) else new_intersection(LETTERS[LETTERS.index(get_col(x))+1], get_row(x))
    }
